<?php
/**
 * Plugin Name: HTS Code Display for WooCommerce (Secure)
...
define('HTS_DISPLAY_VERSION', '1.1.0');
 ...
 */
register_deactivation_hook(__FILE__, 'hts_display_deactivate');
function hts_display_deactivate() {
    // Cleanup if needed
}
    ...
    <div id="hts_code_product_data" class="panel woocommerce_options_panel">
        <div class="options_group">
            ...
            woocommerce_wp_select(array(
                ...
                'desc_tip'    => true,
                'description' => __('Where this product is manufactured', 'hts-display'),
...
function register_hts_rest_fields() {
    ...
    register_rest_field('product', 'country_of_origin', array(
        'get_callback'    => function($product) {
            ...
            return get_post_meta($product['id'], '_country_of_origin', true);
        },
        'update_callback' => function($value, $product) {
            if (!current_user_can('edit_product', $product->ID)) {
                return false;
...
function add_hts_bulk_actions($bulk_actions) {
    ...
    $bulk_actions['clear_hts'] = __('Clear HTS Codes', 'hts-display');
    return $bulk_actions;
}
...
function handle_hts_bulk_actions($redirect_to, $action, $post_ids) {
    ...
    } elseif ($action === 'clear_hts') {
        foreach ($post_ids as $post_id) {
            if (current_user_can('edit_product', $post_id)) {
                delete_post_meta($post_id, '_hts_code');
                delete_post_meta($post_id, '_hts_confidence');
                delete_post_meta($post_id, '_hts_updated');
            }
        }
        $redirect_to = add_query_arg('hts_cleared', count($post_ids), $redirect_to);
    }
...
function hts_bulk_action_notices() {
    ...
    if (!empty($_REQUEST['hts_cleared'])) {
        $count = intval($_REQUEST['hts_cleared']);
        printf(
            '<div class="notice notice-success is-dismissible"><p>' . 
            esc_html(_n('Cleared HTS codes for %s product.', 'Cleared HTS codes for %s products.', $count, 'hts-display')) . 
            '</p></div>',
            $count
        );
    }
...
function hts_dashboard_widget_display() {
    ...
    <div class="hts-dashboard-widget">
        ...
        <p class="hts-actions">
            ...
            <?php if ($products_without > 0): ?>
                <a href="#" class="button button-primary" style="margin-left: 5px;">
                    <?php esc_html_e('Run HTS Matcher', 'hts-display'); ?>
                </a>
            <?php endif; ?>
        </p>
    </div>
...
// ===========================
// 9. AJAX HANDLER FOR HTS LOOKUP
// ===========================

add_action('wp_ajax_hts_lookup_code', 'ajax_hts_lookup_code');
function ajax_hts_lookup_code() {
    // Check nonce
    check_ajax_referer('hts_ajax_nonce', 'nonce');
    
    // Check capabilities
    if (!current_user_can('edit_products')) {
        wp_die(__('Unauthorized', 'hts-display'));
    }
    
    $hts_code = isset($_POST['hts_code']) ? sanitize_text_field($_POST['hts_code']) : '';
    
    if (!hts_validate_code_format($hts_code)) {
        wp_send_json_error(__('Invalid HTS code format', 'hts-display'));
    }
    
    // Here you could make an API call to validate or get more info about the HTS code
...
// Add AJAX script and nonce
add_action('admin_enqueue_scripts', 'hts_admin_scripts');
function hts_admin_scripts($hook) {
    if ('post.php' === $hook || 'post-new.php' === $hook) {
        global $post_type;
        if ('product' === $post_type) {
            wp_localize_script('wc-admin-product-meta-boxes', 'hts_ajax', array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce'    => wp_create_nonce('hts_ajax_nonce'),
            ));
        }
    }
}