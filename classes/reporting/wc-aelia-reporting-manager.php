...
class WC_Aelia_Reporting_Manager implements IWC_Aelia_Reporting_Manager {
	...
	/**
	 * Returns the SQL to fetch the list of currencies from existing orders.
	 * This method also supports the HPOS tables.
	 *
	 * @return string
	 * @since 5.0.1.230329
	 * @link https://bitbucket.org/businessdad/woocommerce-currency-switcher/issues/edit/27
	 */
	protected static function get_currencies_from_sales_query(): string {
		global $wpdb;

		if(aelia_is_hpos_feature_enabled()) {
			// If the HPOS feature is enabled, fetch the list of currencies from the new
			// orders table
			...
			// If the HPOS feature is disabled, fetch the list of currencies from the post meta
			// table
			$SQL = "
				SELECT DISTINCT
	...
	public static function get_currencies_from_sales() {
		if(empty(self::$currencies_from_sales)) {
			// Allow 3rd parties to set the list of currencies for the reports. This allows large shops
			// to skip the SQL query to fetch the currencies
			// @since 4.15.4.230322
			$pre_get_currencies_from_sales = apply_filters('wc_aelia_cs_reports_pre_get_currencies_from_sales', []);
			if(!empty($pre_get_currencies_from_sales) && is_array($pre_get_currencies_from_sales)) {
				self::$currencies_from_sales = WC_Aelia_Currencies_Manager::get_currency_names($pre_get_currencies_from_sales);
			}
		}

		// If the list of currencies is still empty, fetch it from the database
		if(empty(self::$currencies_from_sales)) {
			...
			if(empty($currencies)) {
				...
				$currencies = array_merge($wpdb->get_col(self::get_currencies_from_sales_query()),
