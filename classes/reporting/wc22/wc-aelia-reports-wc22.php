...
class Reports extends \Aelia\WC\CurrencySwitcher\Reports {
	...
	public function woocommerce_reports_get_order_report_data_args($report_args) {
		...
		// If a valid currency, other than the default value (i.e. "base") was selected for the reports,
		// only show data for orders in that currency
		//
		// @since 4.15.4.230322
		// @link https://bitbucket.org/businessdad/woocommerce-currency-switcher/issues/36
		if(!empty($report_currency) && ($report_currency !== Definitions::DEF_REPORT_CURRENCY) && in_array($report_currency, array_keys(WC_Aelia_Reporting_Manager::get_currencies_from_sales()))) {
